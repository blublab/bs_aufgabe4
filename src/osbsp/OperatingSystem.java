...
public class OperatingSystem {
  ...
  /**
   * Seitengröße (default: 2^8 Byte = 256 Byte Seitengröße --> max. 2^8 = 256 Seitenrahmen , 64
  ...
  /**
   *  Max. Anzahl virtueller Seiten: VIRT_ADR_SPACE / PAGE_SIZE (default: 2^12 = 4096 Seiten) 
   */
  private static final int MAX_NO_OF_PAGES = VIRT_ADR_SPACE / PAGE_SIZE;

  /**
   * Platteneigenschaften: Plattengröße = virt. Adressraum reicht hier, weil wir keine weiteren Dateien
   * brauchen 
   */
  ...
  public synchronized int read(int pid, int virtAdr) {
    ...
    // ToDo
	  //getVirtualPageNumber -> vSeite
	  //pageTable des Prozesses holen
	  //processses pageTable.getPte(vSeite)
	  //getOffset
	  // vSeite + Offset -> richtige Adresse?
	  int vpn = getVirtualPageNum(virtAdr);
	  int off = getOffset(virtAdr);
	  int rpfa = getProcess(pid).pageTable.getPte(vpn).realPageFrameAdr;
	  PageTableEntry pte = getProcess(pid).pageTable.getPte(vpn); 
  ...
  private int getVirtualPageNum(int virtAdr) {
    //  TODO
	  // Berechnen der vSeite aus vAdresse
